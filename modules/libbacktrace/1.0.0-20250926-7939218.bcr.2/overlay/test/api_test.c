...
int main(int argc, char **argv) {
  struct callback_state callbacks = {0, 0, 0};
  int pcinfo_result;
  int simple_result;
  int syminfo_result;
  ...
  simple_result = backtrace_simple(state, 0, simple_callback, error_callback,
                                  &callbacks);
  syminfo_result = backtrace_syminfo(state, (uintptr_t)&main, syminfo_callback,
                                     error_callback, &callbacks);
  if (simple_result != 1 || !callbacks.simple_called || !syminfo_result ||
      !callbacks.syminfo_called) {
    fprintf(stderr,
            "pcinfo=%d full=%d simple=%d simple_called=%d syminfo=%d "
            "symbol=%d\n",
            pcinfo_result, callbacks.full_called, simple_result,
            callbacks.simple_called, syminfo_result, callbacks.syminfo_called);
    return 1;
  }
