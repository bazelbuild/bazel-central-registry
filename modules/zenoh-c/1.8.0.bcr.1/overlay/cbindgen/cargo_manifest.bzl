...
_SUPPORTED_LIB_KINDS = ["rust_library", "rust_shared_library", "rust_static_library", "rust_cdylib_library"]
...
def _cargo_manifest_aspect_impl(target, ctx):
    ...
    # available so the synthesized manifest's crate root matches the sources the
    # cbindgen rule feeds in (both come from this crate info).
    ...
    if _is_external_crate(target) and CrateInfo in target:
    ...
    # Get edition - from crate info if available, otherwise use rule attribute or default
    if has_crate_info:
    ...
    if rule.kind in ["rust_shared_library", "rust_cdylib_library"]:
    ...
    ctx.actions.write(
        ...
        content = _CARGO_MANIFEST_TEMPLATE.format(
            ...
            # names. Use the actual crate name when available, otherwise sanitize
            # the label name (e.g. "unwrap-infallible" -> "unwrap_infallible").
            lib_name = crate_info.name if has_crate_info else target.label.name.replace("-", "_"),
