...
# ===========================================================================
# version_script — expose an ELF linker version script as a CcInfo dep.
# ===========================================================================
#
# Bazel's `ada_binary` (and the cc rules) accept CcInfo deps and propagate
# their linking_context — including `user_link_flags` and `additional_inputs`
# — into the final link action. This rule lets us thread an ELF
# `--version-script=` flag (and the script file itself) into the ada_binary
# link action without modifying rules_ada.

def _version_script_impl(ctx):
    script = ctx.file.script
    linker_input = cc_common.create_linker_input(
        owner = ctx.label,
        user_link_flags = depset(["-Wl,--version-script=" + script.path]),
        additional_inputs = depset([script]),
    )
    linking_context = cc_common.create_linking_context(
        linker_inputs = depset([linker_input]),
    )
    return [
        DefaultInfo(files = depset([script])),
        CcInfo(linking_context = linking_context),
    ]

version_script = rule(
    doc = """\
Wrap an ELF linker version script as a `CcInfo` dep so it can be consumed
by `ada_binary` / `cc_binary` / `cc_shared_library` via `deps`. The
script file is added to the link action's inputs and the corresponding
`-Wl,--version-script=` flag is appended to the link command.
""",
    ...
    attrs = {
        "script": attr.label(
            ...
            doc = "The version script file (e.g. `src/grt/grt.ver`).",
        ),
    },
    fragments = ["cpp"],
)
