...
def _arg_map_expand_outputs(value):
    arg, out_map = value
    for part, output in out_map.items():
        arg = arg.replace(part, output.path)
    return arg
...
def _flex_impl(ctx):
    ...
    for arg in ctx.attr.args:
        args.add_all([(ctx.expand_location(arg, ctx.attr.srcs), out_map)], map_each = _arg_map_expand_outputs)
