...
def _libpcap_gen_action(ctx, mnemonic, tool, env):
...
def _libpcap_bison_impl(ctx):
    ...
    return _libpcap_gen_action(
        ctx = ctx,
        mnemonic = "LibpcapBisonGen",
        tool = ctx.executable.bison,
        env = env,
    )

libpcap_bison = rule(
    doc = "An internal rule for running bison in the libpcap project.",
    implementation = _libpcap_bison_impl,
    attrs = {
...
def _libpcap_flex_impl(ctx):
    ...
    return _libpcap_gen_action(
        ctx = ctx,
        mnemonic = "LibpcapFlexGen",
        tool = ctx.executable.flex,
        env = env,
    )

libpcap_flex = rule(
    doc = "An internal rule for running flex in the libpcap project.",
    implementation = _libpcap_flex_impl,
    attrs = {
        "args": attr.string_list(
