...
# The py_binary() is using the local host environment to launch the
# python3 to bootstrap the call execution. If the python3 is not in a fixed
# set of paths, that fails. So we need to allow the actions to see $PATH
NEED_TO_ACCESS_HOST_PYTHON_TO_LAUNCH_PY_BINARY=True
...
def _verilator_astgen_impl(ctx):
    ...
    ctx.actions.run(
        ...
        use_default_shell_env = NEED_TO_ACCESS_HOST_PYTHON_TO_LAUNCH_PY_BINARY,
    )
