...
def _verilator_coverage_internal_test_impl(ctx):
    ...
    coverage_rf = _rf(ctx, ctx.executable.verilator_coverage)
    src_rfs = [_rf(ctx, f) for f in ctx.files.srcs]
    src_args = " ".join(['"${TEST_SRCDIR}/%s"' % s for s in src_rfs])
    expected_rf = _rf(ctx, ctx.file.expected)

    content = "\n".join([
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        'COVERAGE="${TEST_SRCDIR}/%s"' % coverage_rf,
        'OUT="${TEST_TMPDIR}/coverage.info"',
        '"${COVERAGE}" --write-info "${OUT}" ' + src_args,
        'diff -u "${TEST_SRCDIR}/%s" "${OUT}"' % expected_rf,
    ])

    ctx.actions.write(output = script, content = content, is_executable = True)

    runfiles = ctx.runfiles(
        files = ctx.files.srcs + [ctx.file.expected, ctx.executable.verilator_coverage],
    )
...
verilator_coverage_internal_test = rule(
    ...
    attrs = {
        "verilator_coverage": attr.label(
            executable = True,
            cfg = "target",
            default = "//:verilator_coverage_bin",
        ),
        "srcs": attr.label_list(allow_files = [".dat"]),
        "expected": attr.label(
            allow_single_file = True,
            mandatory = True,
        ),
