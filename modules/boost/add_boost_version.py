...
def find_closest_version(target_version: str, available_versions: list[str]) -> str:
    ...
    # If no lower version, use the closest higher version
...
def run_buildifier(directory: Path) -> None:
    """Run buildifier to format and lint Bazel files in a directory.

    Args:
        directory: Directory to run buildifier on
    """
    subprocess.run(
        [
            "buildifier",
...
def copy_and_update_directory(source_path: Path, target_path: Path, old_version: str, new_version: str) -> None:
    ...
    try:
        ...
        logging.warning("Could not run buildifier on %s: %s", target_path, e)
...
def generate_meta_module_files(version: str, modules_dir: Path, boost_modules: list[str]) -> bool:
    ...
    main_module_file.write_text(module_content)
...
def main() -> None:
    ...
    for module in boost_modules:
        ...
        if args.version in module_versions:
            # Module already has this version
            logging.debug("Retaining existing version: %s", module)
            modules_with_version[module] = False
            continue

        # Need to create this version
        source_ver = find_closest_version(args.version, module_versions)
        assert source_ver, f"Failed to find previous version for {module}"

        src_path = module_path / source_ver
        tgt_path = module_path / args.version

        logging.debug("Promoting %s (%s -> %s)", module, source_ver, args.version)
    ...
    for module, should_update in modules_with_version.items():
        if should_update:
