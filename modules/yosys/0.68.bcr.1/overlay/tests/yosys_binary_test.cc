...
/**
 * The SDC passes (techlibs/common/opensta.cc, sdc_expand.cc) are easy to
 * lose on a version bump -- they live outside passes/ and joined upstream's
 * default build late. `help <cmd>` errors out on an unregistered command.
 */
TEST_F(YosysTest, SdcPassesRegistered) {
    std::string cmd = yosys_path + " -p 'help sdc_expand' 2>&1";
    std::string output = exec(cmd);

    EXPECT_TRUE(output.find("sdc_expand") != std::string::npos)
        << "sdc_expand pass not registered: " << output;

    cmd = yosys_path + " -p 'help opensta' 2>&1";
    output = exec(cmd);

    EXPECT_TRUE(output.find("opensta") != std::string::npos)
        << "opensta pass not registered: " << output;
}
