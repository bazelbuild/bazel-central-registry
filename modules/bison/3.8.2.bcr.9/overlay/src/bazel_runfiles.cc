...
#include <string.h> // POSIX strdup

#include "rules_cc/cc/runfiles/runfiles.h"

// Bazel auto-injects BAZEL_CURRENT_REPOSITORY for targets that depend on the
// runfiles library, but only recognizes @rules_cc//cc/runfiles on Bazel 7.4+
// (older versions still expect the legacy @bazel_tools label). Fall back to
// the main repository so compilation succeeds; the rlocationpaths consumed
// below are already canonical, so the source_repository value is unused for
// lookup.
...
static struct BazelRunfiles* bazel_runfiles = nullptr;
...
char* bazel_runfiles_bison_pkgdatadir() {
    ...
    const char* rlocationpath = std::getenv("BISON_PKGDATADIR_RLOCATIONPATH");
...
char* bazel_runfiles_m4() {
    ...
    const char* m4_rlocationpath = std::getenv("M4_RLOCATIONPATH");
