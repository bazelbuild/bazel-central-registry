...
load("@rules_cc//cc:defs.bzl", "CcInfo", "cc_binary", "cc_library", "cc_test")
load(
    ":component_defs.bzl",
...
def _select_label(comp):
    """Return the config label for a component.

    Uses the _or_needed group when the component has transitive select
    ancestors, so that source files compile automatically when a parent codec
    is enabled.
...
def _config_copt(prefix, comp, value):
    return "-DFFMPEG_BAZEL_{}_{}={}".format(prefix, comp.upper(), value)

def component_config_copts(components):
    """Emit copts that vary with component build settings.

    Bazel writes generated FFmpeg config/list files to stable output paths.
    Carrying the relevant selected state in the C compile command forces
    recompilation when those generated files change between flag combinations.

    Args:
      components: List of component names to emit copts for.

    Returns:
      A concatenation of select() values suitable for use in cc_library copts.
    """
    result = []
    for comp in components:
        result = result + select({
            _select_label(comp): [_config_copt("COMPONENT", comp, 1)],
            "//conditions:default": [_config_copt("COMPONENT", comp, 0)],
        })
    return result

def flag_config_copts(flags):
    """Emit copts that vary with direct bool_flag build settings.

    Args:
      flags: List of bool_flag names to emit copts for.
    ...
    for flag in flags:
        result = result + select({
            "//:enable_{}_is_true".format(flag): [_config_copt("FLAG", flag, 1)],
            "//conditions:default": [_config_copt("FLAG", flag, 0)],
        })
    return result
...
def _forward_test_impl(ctx):
    target = ctx.attr.target[0]
    original = target[DefaultInfo]
    ctx.actions.symlink(
        output = ctx.outputs.executable,
        target_file = original.files_to_run.executable,
        is_executable = True,
    )
    ...
    providers = [DefaultInfo(
        ...
        runfiles = runfiles.merge(original.default_runfiles),
    )]
...
with_defaults_test = rule(
    doc = "Forwards a cc_test target built with platform-appropriate component defaults.",
    implementation = _forward_test_impl,
    attrs = {
        "cpu": attr.string(
            doc = "Target CPU, resolved via select() on @platforms//cpu. Reserved for future use.",
        ),
        ...
        "target": attr.label(
            doc = "The cc_test target to rebuild under platform-aware defaults.",
            cfg = _with_defaults_transition,
            executable = True,
            mandatory = True,
        ),
...
def cc_variant_test(name, **kwargs):
    """Like cc_test, but also emits a with_defaults/ variant.

    The whole test — not just its dependencies — is rebuilt under the
    transition, so the test's own compilation sees the same generated headers
    as the libraries it links against.

    Args:
      name: Target name, forwarded to cc_test.
      **kwargs: All remaining arguments are forwarded to cc_test.
    """
