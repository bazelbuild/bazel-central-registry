...
# FFmpeg's configure derives the target architecture from the compiler triple.
# Here it is probed with the preprocessor, so every predicate has to accept both
# the GCC/Clang spelling and the MSVC spelling of the same architecture.
_IS_AARCH64 = "defined(__aarch64__) || defined(_M_ARM64)"
_IS_ARM = "defined(__arm__) || defined(_M_ARM)"
_IS_X86_32 = "defined(__i386__) || defined(_M_IX86)"
_IS_X86_64 = "defined(__x86_64__) || defined(_M_X64)"
_IS_X86 = _IS_X86_32 + " || " + _IS_X86_64

_X86_ARCH_GUARD = "#if !(" + _IS_X86 + ")\n#error wrong arch\n#endif\n"

def _x86_inline(instruction):
    """Body for an x86 inline-assembly probe (configure's check_inline_asm).

    Args:
        instruction: The assembly instruction to probe, in GNU (AT&T) syntax.

    Returns:
        Probe source for AC_TRY_COMPILE.
    """
    return _X86_ARCH_GUARD + "void foo(void){ __asm__ volatile(\"" + instruction + "\"); }"

def _x86_external(instruction):
    """Body for an x86 external-assembly probe (configure's check_x86asm).

    External assembly is built by nasm, not by the C compiler, so the GNU
    inline-asm probe only says something meaningful for GCC and Clang. MSVC has
    no GNU inline assembly; there the x86 arch guard alone decides the result.
    ...
    Returns:
        Probe source for AC_TRY_COMPILE.
    """
    return _X86_ARCH_GUARD + "#if defined(__GNUC__)\nvoid foo(void){ __asm__ volatile(\"" + instruction + "\"); }\n#endif"

# The Winsock headers, plus the linker directive that makes the symbols they
# declare resolvable — MSVC has no command line in these probes to pass
# ws2_32.lib on.
_WINSOCK_INCLUDES = [
    "#include <winsock2.h>",
    "#include <ws2tcpip.h>",
...
def _net_includes(posix_includes):
    """Includes for a socket probe.

    Once configure detects winsock2.h it re-runs its socket checks against
    winsock2.h/ws2tcpip.h instead of the POSIX headers; the same choice is made
    here with the preprocessor so a single check works on every target.

    Args:
        posix_includes: The #include lines to use on non-Windows targets.

    Returns:
        An include list for AC_TRY_COMPILE / AC_TRY_LINK.
    """
    return ["#ifdef _WIN32"] + _WINSOCK_INCLUDES + ["#else"] + posix_includes + ["#endif"]

FFMPEG_CONFIG_CHECKS = [
    # ========== Architecture detection (ARCH_LIST) ==========
    checks.AC_TRY_COMPILE(name = "ac_cv_arch_aarch64", define = "ARCH_AARCH64", code = "#if !(" + _IS_AARCH64 + ")\n#error\n#endif"),
    checks.AC_TRY_COMPILE(define = "ARCH_ARM", code = "#if !(" + _IS_ARM + ")\n#error\n#endif"),
    ...
    checks.AC_TRY_COMPILE(define = "ARCH_WASM", code = "#if !defined(__wasm__)\n#error\n#endif"),
    checks.AC_TRY_COMPILE(define = "ARCH_X86", code = "#if !(" + _IS_X86 + ")\n#error\n#endif"),
    checks.AC_TRY_COMPILE(define = "ARCH_X86_32", code = "#if !(" + _IS_X86_32 + ")\n#error\n#endif"),
    checks.AC_TRY_COMPILE(define = "ARCH_X86_64", code = "#if !(" + _IS_X86_64 + ")\n#error\n#endif"),

    # ========== OS metadata ==========
    checks.AC_TRY_COMPILE(name = "ac_cv_os_darwin", code = "#if !defined(__APPLE__)\n#error\n#endif"),
    checks.AC_TRY_COMPILE(name = "ac_cv_os_windows", code = "#if !defined(_WIN32)\n#error\n#endif"),
    ...
    # Win64 and ELF have no symbol prefix; only Mach-O does.
    ...
    # Probed by calling the function rather than with the `(long)func` cast the
    # rest of this list uses. That cast is upstream configure's idiom, but
    # upstream only ever feeds it to GCC and Clang. On LLP64 Windows `long` is
    # 32 bits and a function pointer is 64, so the cast truncates; C compilers
    # warn, C++ compilers reject it outright.
    #
    # Getting this one wrong is not a missing feature but a silent miscompile:
    # av_malloc() falls through to plain malloc() (libavutil/mem.c:110), which
    # aligns to 16 on Windows, and every `mova` in FFmpeg's AVX2 kernels then
    # faults on a buffer that only happens to be 32-byte aligned. Meanwhile
    # _aligned_malloc is unconditionally present in the UCRT and in mingw-w64,
    # so on Windows a "no" answer here can only ever be a broken probe.
    #
    # It was exactly that on the BCR Windows machine, which failed this probe on
    # one run and a different arbitrary handful of AC_TRY_LINK probes on the
    # next: rules_cc_autoconf ran cl.exe without /Fo, so concurrent
    # configurations raced on one intermediate object in the shared execroot.
    # Fixed in rules_cc_autoconf 0.20.0, which is why this is no longer OR'd
    # with windows.h.
    checks.AC_TRY_LINK(name = "ac_cv_aligned_malloc", code = "void *p = _aligned_malloc(1, 16); _aligned_free(p);", includes = ["#include <malloc.h>"]),
    ...
    checks.AC_TRY_LINK(define = "HAVE_CLOSESOCKET", code = "long x = (long)closesocket; (void)x;", includes = _WINSOCK_INCLUDES),
    # AC_CHECK_LIB rather than AC_TRY_LINK because the symbol needs a library
    # named: CommandLineToArgvW lives in shell32.dll and shellapi.h carries no
    # `#pragma comment(lib, "shell32.lib")`, so an AC_TRY_LINK probe is a false
    # negative on every toolchain. AC_CHECK_LIB passes the library as a bare
    # positional, which both cl and the GNU drivers forward to the linker.
    # Upstream configure does the same thing (`check_lib shell32 ... -lshell32`).
    # Without this, fftools/cmdutils.c compiles out its wchar argv path and the
    # Windows CLI mangles non-ASCII arguments.
    checks.AC_CHECK_LIB("shell32", "CommandLineToArgvW", define = "HAVE_COMMANDLINETOARGVW"),
    ...
    checks.AC_TRY_LINK(define = "HAVE_GETADDRINFO", code = "long x = (long)getaddrinfo; (void)x;", includes = _net_includes(["#include <sys/types.h>", "#include <sys/socket.h>", "#include <netdb.h>"])),
    ...
    checks.AC_TRY_LINK(define = "HAVE_IOCTL", code = "long x = (long)ioctl; (void)x;", includes = ["#include <sys/ioctl.h>"]),
    ...
    checks.AC_TRY_LINK(define = "HAVE_PTHREAD_SETAFFINITY_NP", code = "long x = (long)pthread_setaffinity_np; (void)x;", includes = ["#define _GNU_SOURCE 1", "#include <pthread.h>"]),
    ...
    checks.AC_TRY_LINK(define = "HAVE_SIGACTION", code = "long x = (long)sigaction; (void)x;", includes = ["#include <signal.h>"]),
    checks.AC_TRY_LINK(define = "HAVE_SIGLONGJMP", code = "long x = (long)siglongjmp; (void)x;", includes = ["#include <setjmp.h>"]),
    ...
    checks.AC_TRY_COMPILE(define = "HAVE_SOCKLEN_T", code = "socklen_t x = 0; (void)x;", includes = _net_includes(["#include <sys/types.h>", "#include <sys/socket.h>"])),
    checks.AC_TRY_COMPILE(define = "HAVE_STRUCT_ADDRINFO", code = "struct addrinfo x; (void)x;", includes = _net_includes(["#include <netdb.h>"])),
    checks.AC_TRY_COMPILE(define = "HAVE_STRUCT_GROUP_SOURCE_REQ", code = "struct group_source_req x; (void)x;", includes = _net_includes(["#include <netinet/in.h>"])),
    checks.AC_TRY_COMPILE(define = "HAVE_STRUCT_IP_MREQ_SOURCE", code = "struct ip_mreq_source x; (void)x;", includes = _net_includes(["#include <netinet/in.h>"])),
    checks.AC_TRY_COMPILE(define = "HAVE_STRUCT_IPV6_MREQ", code = "struct ipv6_mreq x; (void)x;", includes = _net_includes(["#include <netinet/in.h>"])),
    ...
    checks.AC_TRY_COMPILE(define = "HAVE_STRUCT_MSGHDR_MSG_FLAGS", code = "const void *p = &((struct msghdr *)0)->msg_flags; (void)p;", includes = _net_includes(["#include <sys/socket.h>"])),
    checks.AC_TRY_COMPILE(define = "HAVE_STRUCT_POLLFD", code = "struct pollfd x; (void)x;", includes = _net_includes(["#include <poll.h>"])),
    ...
    checks.AC_TRY_COMPILE(define = "HAVE_STRUCT_SOCKADDR_IN6", code = "struct sockaddr_in6 x; (void)x;", includes = _net_includes(["#include <netinet/in.h>"])),
    checks.AC_TRY_COMPILE(define = "HAVE_STRUCT_SOCKADDR_SA_LEN", code = "const void *p = &((struct sockaddr *)0)->sa_len; (void)p;", includes = _net_includes(["#include <sys/types.h>", "#include <sys/socket.h>"])),
    checks.AC_TRY_COMPILE(define = "HAVE_STRUCT_SOCKADDR_STORAGE", code = "struct sockaddr_storage x; (void)x;", includes = _net_includes(["#include <sys/types.h>", "#include <sys/socket.h>"])),
    ...
    checks.AC_TRY_COMPILE(define = "HAVE_KCMVIDEOCODECTYPE_AV1", code = "int x = (int)kCMVideoCodecType_AV1; (void)x;", includes = ["#include <CoreMedia/CMFormatDescription.h>"]),
    checks.AC_TRY_COMPILE(define = "HAVE_KCVPIXELFORMATTYPE_422YPCBCR8_YUVS", code = "int x = (int)kCVPixelFormatType_422YpCbCr8_yuvs; (void)x;", includes = ["#include <CoreVideo/CVPixelBuffer.h>"]),
    checks.AC_TRY_COMPILE(define = "HAVE_KVTQPMODULATIONLEVEL_DEFAULT", code = "int x = (int)kVTQPModulationLevel_Default; (void)x;", includes = ["#include <VideoToolbox/VTCompressionProperties.h>"]),

    # ========== ISA extension checks (ARCH_EXT_LIST) ==========
    # SVE and SME objects are not built by this overlay. Their HAVE_* macros
    # retain the zero defaults in config.h.in on every target.
    checks.AC_TRY_COMPILE(name = "ac_cv_armv5te_inline", define = "HAVE_ARMV5TE_INLINE", code = "#if !(" + _IS_ARM + ")\n#error wrong arch\n#endif\nvoid foo(void){ __asm__ volatile(\"qadd r0, r0, r0\"); }"),
    checks.AC_TRY_COMPILE(define = "HAVE_ARMV5TE_EXTERNAL", code = "#if !(" + _IS_ARM + ")\n#error wrong arch\n#endif\nvoid foo(void){ __asm__ volatile(\"qadd r0, r0, r0\"); }"),
    checks.AC_DEFINE("HAVE_ARMV5TE", condition = "ac_cv_armv5te_inline", if_true = "1", if_false = "0"),
    checks.AC_TRY_COMPILE(name = "ac_cv_armv6_inline", define = "HAVE_ARMV6_INLINE", code = "#if !(" + _IS_ARM + ")\n#error wrong arch\n#endif\nvoid foo(void){ __asm__ volatile(\"sadd16 r0, r0, r0\"); }"),
    checks.AC_TRY_COMPILE(define = "HAVE_ARMV6_EXTERNAL", code = "#if !(" + _IS_ARM + ")\n#error wrong arch\n#endif\nvoid foo(void){ __asm__ volatile(\"sadd16 r0, r0, r0\"); }"),
    checks.AC_DEFINE("HAVE_ARMV6", condition = "ac_cv_armv6_inline", if_true = "1", if_false = "0"),
    checks.AC_TRY_COMPILE(name = "ac_cv_armv6t2_inline", define = "HAVE_ARMV6T2_INLINE", code = "#if !(" + _IS_ARM + ")\n#error wrong arch\n#endif\nvoid foo(void){ __asm__ volatile(\"movt r0, #0\"); }"),
    checks.AC_TRY_COMPILE(define = "HAVE_ARMV6T2_EXTERNAL", code = "#if !(" + _IS_ARM + ")\n#error wrong arch\n#endif\nvoid foo(void){ __asm__ volatile(\"movt r0, #0\"); }"),
    checks.AC_DEFINE("HAVE_ARMV6T2", condition = "ac_cv_armv6t2_inline", if_true = "1", if_false = "0"),
    checks.AC_TRY_COMPILE(name = "ac_cv_armv8_inline", define = "HAVE_ARMV8_INLINE", code = "#if !(" + _IS_AARCH64 + ")\n#error wrong arch\n#endif\nvoid foo(void){ __asm__ volatile(\"prfm pldl1strm, [x0]\"); }"),
    checks.AC_TRY_COMPILE(define = "HAVE_ARMV8_EXTERNAL", code = "#if !(" + _IS_AARCH64 + ")\n#error wrong arch\n#endif\nvoid foo(void){ __asm__ volatile(\"prfm pldl1strm, [x0]\"); }"),
    checks.AC_DEFINE("HAVE_ARMV8", condition = "ac_cv_armv8_inline", if_true = "1", if_false = "0"),
    checks.AC_TRY_COMPILE(name = "ac_cv_arm_crc_inline", define = "HAVE_ARM_CRC_INLINE", code = "#if !(" + _IS_AARCH64 + ")\n#error wrong arch\n#endif\nvoid foo(void){ __asm__ volatile(\"crc32x w0, w0, x0\"); }"),
    checks.AC_TRY_COMPILE(define = "HAVE_ARM_CRC_EXTERNAL", code = "#if !(" + _IS_AARCH64 + ")\n#error wrong arch\n#endif\nvoid foo(void){ __asm__ volatile(\"crc32x w0, w0, x0\"); }"),
    checks.AC_DEFINE("HAVE_ARM_CRC", condition = "ac_cv_arm_crc_inline", if_true = "1", if_false = "0"),
    checks.AC_TRY_COMPILE(name = "ac_cv_pmull_inline", define = "HAVE_PMULL_INLINE", code = "#if !(" + _IS_AARCH64 + ")\n#error wrong arch\n#endif\nvoid foo(void){ __asm__ volatile(\"pmull v0.1q, v0.1d, v0.1d\"); }"),
    checks.AC_TRY_COMPILE(define = "HAVE_PMULL_EXTERNAL", code = "#if !(" + _IS_AARCH64 + ")\n#error wrong arch\n#endif\nvoid foo(void){ __asm__ volatile(\"pmull v0.1q, v0.1d, v0.1d\"); }"),
    checks.AC_DEFINE("HAVE_PMULL", condition = "ac_cv_pmull_inline", if_true = "1", if_false = "0"),
    checks.AC_TRY_COMPILE(name = "ac_cv_eor3_inline", define = "HAVE_EOR3_INLINE", code = "#if !(" + _IS_AARCH64 + ")\n#error wrong arch\n#endif\nvoid foo(void){ __asm__ volatile(\"eor3 v0.16b, v1.16b, v2.16b, v3.16b\"); }"),
    checks.AC_TRY_COMPILE(define = "HAVE_EOR3_EXTERNAL", code = "#if !(" + _IS_AARCH64 + ")\n#error wrong arch\n#endif\nvoid foo(void){ __asm__ volatile(\"eor3 v0.16b, v1.16b, v2.16b, v3.16b\"); }"),
    checks.AC_DEFINE("HAVE_EOR3", condition = "ac_cv_eor3_inline", if_true = "1", if_false = "0"),
    checks.AC_TRY_COMPILE(name = "ac_cv_dotprod_inline", define = "HAVE_DOTPROD_INLINE", code = "#if !(" + _IS_AARCH64 + ")\n#error wrong arch\n#endif\nvoid foo(void){ __asm__ volatile(\"udot v0.4s, v0.16b, v0.16b\"); }"),
    checks.AC_TRY_COMPILE(define = "HAVE_DOTPROD_EXTERNAL", code = "#if !(" + _IS_AARCH64 + ")\n#error wrong arch\n#endif\nvoid foo(void){ __asm__ volatile(\"udot v0.4s, v0.16b, v0.16b\"); }"),
    checks.AC_DEFINE("HAVE_DOTPROD", condition = "ac_cv_dotprod_inline", if_true = "1", if_false = "0"),
    checks.AC_TRY_COMPILE(name = "ac_cv_i8mm_inline", define = "HAVE_I8MM_INLINE", code = "#if !(" + _IS_AARCH64 + ")\n#error wrong arch\n#endif\nvoid foo(void){ __asm__ volatile(\"usdot v0.4s, v0.16b, v0.16b\"); }"),
    checks.AC_TRY_COMPILE(define = "HAVE_I8MM_EXTERNAL", code = "#if !(" + _IS_AARCH64 + ")\n#error wrong arch\n#endif\nvoid foo(void){ __asm__ volatile(\"usdot v0.4s, v0.16b, v0.16b\"); }"),
    checks.AC_DEFINE("HAVE_I8MM", condition = "ac_cv_i8mm_inline", if_true = "1", if_false = "0"),
    checks.AC_TRY_COMPILE(name = "ac_cv_neon_inline", define = "HAVE_NEON_INLINE", code = "#if !(" + _IS_AARCH64 + ")\n#error wrong arch\n#endif\nvoid foo(void){ __asm__ volatile(\"ext v0.8B, v0.8B, v1.8B, #1\"); }"),
    checks.AC_TRY_COMPILE(define = "HAVE_NEON_EXTERNAL", code = "#if !(" + _IS_AARCH64 + ")\n#error wrong arch\n#endif\nvoid foo(void){ __asm__ volatile(\"ext v0.8B, v0.8B, v1.8B, #1\"); }"),
    checks.AC_DEFINE("HAVE_NEON", condition = "ac_cv_neon_inline", if_true = "1", if_false = "0"),
    checks.AC_TRY_COMPILE(name = "ac_cv_vfp_inline", define = "HAVE_VFP_INLINE", code = "#if !(" + _IS_ARM + ")\n#error wrong arch\n#endif\nvoid foo(void){ __asm__ volatile(\"fadds s0, s0, s0\"); }"),
    checks.AC_TRY_COMPILE(define = "HAVE_VFP_EXTERNAL", code = "#if !(" + _IS_ARM + ")\n#error wrong arch\n#endif\nvoid foo(void){ __asm__ volatile(\"fadds s0, s0, s0\"); }"),
    checks.AC_DEFINE("HAVE_VFP", condition = "ac_cv_vfp_inline", if_true = "1", if_false = "0"),
    checks.AC_TRY_COMPILE(name = "ac_cv_vfpv3_inline", define = "HAVE_VFPV3_INLINE", code = "#if !(" + _IS_ARM + ")\n#error wrong arch\n#endif\nvoid foo(void){ __asm__ volatile(\"vmov.f32 s0, #1.0\"); }"),
    checks.AC_TRY_COMPILE(define = "HAVE_VFPV3_EXTERNAL", code = "#if !(" + _IS_ARM + ")\n#error wrong arch\n#endif\nvoid foo(void){ __asm__ volatile(\"vmov.f32 s0, #1.0\"); }"),
    checks.AC_DEFINE("HAVE_VFPV3", condition = "ac_cv_vfpv3_inline", if_true = "1", if_false = "0"),
    checks.AC_TRY_COMPILE(name = "ac_cv_setend_inline", define = "HAVE_SETEND_INLINE", code = "#if !(" + _IS_ARM + ")\n#error wrong arch\n#endif\nvoid foo(void){ __asm__ volatile(\"setend be\"); }"),
    checks.AC_TRY_COMPILE(define = "HAVE_SETEND_EXTERNAL", code = "#if !(" + _IS_ARM + ")\n#error wrong arch\n#endif\nvoid foo(void){ __asm__ volatile(\"setend be\"); }"),
    ...
    checks.AC_TRY_COMPILE(name = "ac_cv_aesni_inline", define = "HAVE_AESNI_INLINE", code = _x86_inline("aesenc %xmm0, %xmm0")),
    checks.AC_TRY_COMPILE(name = "ac_cv_aesni_external", define = "HAVE_AESNI_EXTERNAL", code = _x86_external("aesenc %xmm0, %xmm0")),
    checks.AC_DEFINE("HAVE_AESNI", condition = "ac_cv_aesni_external", if_true = "1", if_false = "0"),
    checks.AC_TRY_COMPILE(name = "ac_cv_clmul_inline", define = "HAVE_CLMUL_INLINE", code = _x86_inline("pclmulqdq $0, %xmm0, %xmm0")),
    checks.AC_TRY_COMPILE(name = "ac_cv_clmul_external", define = "HAVE_CLMUL_EXTERNAL", code = _x86_external("pclmulqdq $0, %xmm0, %xmm0")),
    checks.AC_DEFINE("HAVE_CLMUL", condition = "ac_cv_clmul_external", if_true = "1", if_false = "0"),
    checks.AC_TRY_COMPILE(name = "ac_cv_amd3dnow_inline", define = "HAVE_AMD3DNOW_INLINE", code = _x86_inline("pfadd %mm0, %mm1")),
    checks.AC_TRY_COMPILE(name = "ac_cv_amd3dnow_external", define = "HAVE_AMD3DNOW_EXTERNAL", code = _x86_external("pfadd %mm0, %mm1")),
    checks.AC_DEFINE("HAVE_AMD3DNOW", condition = "ac_cv_amd3dnow_external", if_true = "1", if_false = "0"),
    checks.AC_TRY_COMPILE(name = "ac_cv_amd3dnowext_inline", define = "HAVE_AMD3DNOWEXT_INLINE", code = _x86_inline("pfnacc %mm0, %mm1")),
    checks.AC_TRY_COMPILE(name = "ac_cv_amd3dnowext_external", define = "HAVE_AMD3DNOWEXT_EXTERNAL", code = _x86_external("pfnacc %mm0, %mm1")),
    checks.AC_DEFINE("HAVE_AMD3DNOWEXT", condition = "ac_cv_amd3dnowext_external", if_true = "1", if_false = "0"),
    checks.AC_TRY_COMPILE(name = "ac_cv_avx_inline", define = "HAVE_AVX_INLINE", code = _x86_inline("vextractf128 $0, %ymm0, %xmm0")),
    checks.AC_TRY_COMPILE(name = "ac_cv_avx_external", define = "HAVE_AVX_EXTERNAL", code = _x86_external("vextractf128 $0, %ymm0, %xmm0")),
    checks.AC_DEFINE("HAVE_AVX", condition = "ac_cv_avx_external", if_true = "1", if_false = "0"),
    checks.AC_TRY_COMPILE(name = "ac_cv_avx2_inline", define = "HAVE_AVX2_INLINE", code = _x86_inline("vextracti128 $0, %ymm0, %xmm0")),
    checks.AC_TRY_COMPILE(name = "ac_cv_avx2_external", define = "HAVE_AVX2_EXTERNAL", code = _x86_external("vextracti128 $0, %ymm0, %xmm0")),
    checks.AC_DEFINE("HAVE_AVX2", condition = "ac_cv_avx2_external", if_true = "1", if_false = "0"),
    checks.AC_TRY_COMPILE(name = "ac_cv_avx512_inline", define = "HAVE_AVX512_INLINE", code = _x86_inline("vmovdqa32 %zmm0, %zmm0")),
    checks.AC_TRY_COMPILE(name = "ac_cv_avx512_external", define = "HAVE_AVX512_EXTERNAL", code = _x86_external("vmovdqa32 %zmm0, %zmm0")),
    checks.AC_DEFINE("HAVE_AVX512", condition = "ac_cv_avx512_external", if_true = "1", if_false = "0"),
    checks.AC_TRY_COMPILE(name = "ac_cv_avx512icl_inline", define = "HAVE_AVX512ICL_INLINE", code = _x86_inline("vpdpwssds %zmm0, %zmm1, %zmm2")),
    checks.AC_TRY_COMPILE(name = "ac_cv_avx512icl_external", define = "HAVE_AVX512ICL_EXTERNAL", code = _x86_external("vpdpwssds %zmm0, %zmm1, %zmm2")),
    checks.AC_DEFINE("HAVE_AVX512ICL", condition = "ac_cv_avx512icl_external", if_true = "1", if_false = "0"),
    checks.AC_TRY_COMPILE(name = "ac_cv_fma3_inline", define = "HAVE_FMA3_INLINE", code = _x86_inline("vfmadd132ps %ymm0, %ymm1, %ymm2")),
    checks.AC_TRY_COMPILE(name = "ac_cv_fma3_external", define = "HAVE_FMA3_EXTERNAL", code = _x86_external("vfmadd132ps %ymm0, %ymm1, %ymm2")),
    checks.AC_DEFINE("HAVE_FMA3", condition = "ac_cv_fma3_external", if_true = "1", if_false = "0"),
    checks.AC_TRY_COMPILE(name = "ac_cv_fma4_inline", define = "HAVE_FMA4_INLINE", code = _x86_inline("vfmaddps %ymm0, %ymm1, %ymm2, %ymm3")),
    checks.AC_TRY_COMPILE(name = "ac_cv_fma4_external", define = "HAVE_FMA4_EXTERNAL", code = _x86_external("vfmaddps %ymm0, %ymm1, %ymm2, %ymm3")),
    checks.AC_DEFINE("HAVE_FMA4", condition = "ac_cv_fma4_external", if_true = "1", if_false = "0"),
    checks.AC_TRY_COMPILE(name = "ac_cv_mmx_inline", define = "HAVE_MMX_INLINE", code = _x86_inline("emms")),
    checks.AC_TRY_COMPILE(name = "ac_cv_mmx_external", define = "HAVE_MMX_EXTERNAL", code = _x86_external("emms")),
    checks.AC_DEFINE("HAVE_MMX", condition = "ac_cv_mmx_external", if_true = "1", if_false = "0"),
    checks.AC_TRY_COMPILE(name = "ac_cv_mmxext_inline", define = "HAVE_MMXEXT_INLINE", code = _x86_inline("pmaxub %mm0, %mm1")),
    checks.AC_TRY_COMPILE(name = "ac_cv_mmxext_external", define = "HAVE_MMXEXT_EXTERNAL", code = _x86_external("pmaxub %mm0, %mm1")),
    checks.AC_DEFINE("HAVE_MMXEXT", condition = "ac_cv_mmxext_external", if_true = "1", if_false = "0"),
    checks.AC_TRY_COMPILE(name = "ac_cv_sse_inline", define = "HAVE_SSE_INLINE", code = _x86_inline("xorps %xmm0, %xmm0")),
    checks.AC_TRY_COMPILE(name = "ac_cv_sse_external", define = "HAVE_SSE_EXTERNAL", code = _x86_external("xorps %xmm0, %xmm0")),
    checks.AC_DEFINE("HAVE_SSE", condition = "ac_cv_sse_external", if_true = "1", if_false = "0"),
    checks.AC_TRY_COMPILE(name = "ac_cv_sse2_inline", define = "HAVE_SSE2_INLINE", code = _x86_inline("pslld $7, %xmm0")),
    checks.AC_TRY_COMPILE(name = "ac_cv_sse2_external", define = "HAVE_SSE2_EXTERNAL", code = _x86_external("pslld $7, %xmm0")),
    checks.AC_DEFINE("HAVE_SSE2", condition = "ac_cv_sse2_external", if_true = "1", if_false = "0"),
    checks.AC_TRY_COMPILE(name = "ac_cv_sse3_inline", define = "HAVE_SSE3_INLINE", code = _x86_inline("addsubps %xmm0, %xmm1")),
    checks.AC_TRY_COMPILE(name = "ac_cv_sse3_external", define = "HAVE_SSE3_EXTERNAL", code = _x86_external("addsubps %xmm0, %xmm1")),
    checks.AC_DEFINE("HAVE_SSE3", condition = "ac_cv_sse3_external", if_true = "1", if_false = "0"),
    checks.AC_TRY_COMPILE(name = "ac_cv_sse4_inline", define = "HAVE_SSE4_INLINE", code = _x86_inline("pblendvb %xmm0, %xmm1")),
    checks.AC_TRY_COMPILE(name = "ac_cv_sse4_external", define = "HAVE_SSE4_EXTERNAL", code = _x86_external("pblendvb %xmm0, %xmm1")),
    checks.AC_DEFINE("HAVE_SSE4", condition = "ac_cv_sse4_external", if_true = "1", if_false = "0"),
    checks.AC_TRY_COMPILE(name = "ac_cv_sse42_inline", define = "HAVE_SSE42_INLINE", code = _x86_inline("crc32 %eax, %eax")),
    checks.AC_TRY_COMPILE(name = "ac_cv_sse42_external", define = "HAVE_SSE42_EXTERNAL", code = _x86_external("crc32 %eax, %eax")),
    checks.AC_DEFINE("HAVE_SSE42", condition = "ac_cv_sse42_external", if_true = "1", if_false = "0"),
    checks.AC_TRY_COMPILE(name = "ac_cv_ssse3_inline", define = "HAVE_SSSE3_INLINE", code = _x86_inline("pabsw %xmm0, %xmm0")),
    checks.AC_TRY_COMPILE(name = "ac_cv_ssse3_external", define = "HAVE_SSSE3_EXTERNAL", code = _x86_external("pabsw %xmm0, %xmm0")),
    checks.AC_DEFINE("HAVE_SSSE3", condition = "ac_cv_ssse3_external", if_true = "1", if_false = "0"),
    checks.AC_TRY_COMPILE(name = "ac_cv_xop_inline", define = "HAVE_XOP_INLINE", code = _x86_inline("vpmacsdd %xmm0, %xmm1, %xmm2, %xmm3")),
    checks.AC_TRY_COMPILE(name = "ac_cv_xop_external", define = "HAVE_XOP_EXTERNAL", code = _x86_external("vpmacsdd %xmm0, %xmm1, %xmm2, %xmm3")),
    checks.AC_DEFINE("HAVE_XOP", condition = "ac_cv_xop_external", if_true = "1", if_false = "0"),
    checks.AC_TRY_COMPILE(name = "ac_cv_i686_inline", define = "HAVE_I686_INLINE", code = "#if !(" + _IS_X86_32 + ")\n#error wrong arch\n#endif\nvoid foo(void){ __asm__ volatile(\"cmovz %eax, %ebx\"); }"),
    checks.AC_TRY_COMPILE(define = "HAVE_I686_EXTERNAL", code = "#if !(" + _IS_X86_32 + ")\n#error wrong arch\n#endif\nvoid foo(void){ __asm__ volatile(\"cmovz %eax, %ebx\"); }"),
    ...
    checks.AC_TRY_COMPILE(define = "HAVE_ALIGNED_STACK", code = "#if !(" + _IS_X86_64 + " || " + _IS_AARCH64 + ")\n#error\n#endif"),
    checks.AC_TRY_COMPILE(name = "ac_cv_feat_fast_64bit", define = "HAVE_FAST_64BIT", code = "#if !(" + _IS_X86_64 + " || " + _IS_AARCH64 + " || defined(__powerpc64__) || defined(__mips64) || defined(__loongarch64__) || (defined(__riscv) && __riscv_xlen == 64))\n#error\n#endif"),
    checks.AC_TRY_COMPILE(define = "HAVE_FAST_CLZ", code = "#if !(" + _IS_X86 + " || " + _IS_AARCH64 + " || " + _IS_ARM + " || defined(__mips__) || defined(__powerpc__) || defined(__loongarch__) || defined(__riscv))\n#error\n#endif"),
    checks.AC_TRY_COMPILE(define = "HAVE_FAST_CMOV", code = "#if !(" + _IS_X86_64 + " || ((" + _IS_X86_32 + ") && defined(__SSE2__)))\n#error\n#endif"),
    checks.AC_TRY_COMPILE(define = "HAVE_FAST_FLOAT16", code = "#if !(" + _IS_AARCH64 + " || " + _IS_ARM + " || ((" + _IS_X86_64 + ") && defined(__F16C__)))\n#error\n#endif"),
    checks.AC_TRY_COMPILE(define = "HAVE_SIMD_ALIGN_16", code = "#if !(" + _IS_X86_64 + " || " + _IS_AARCH64 + " || " + _IS_ARM + " || defined(__powerpc__) || defined(__mips__))\n#error\n#endif"),
    # Kept next to HAVE_SIMD_ALIGN_* because the three have to agree: those two
    # decide what alignment av_malloc() asks for, and this one decides whether
    # anything honours the request. If they disagree, libavutil/mem.c quietly
    # hands 16-byte-aligned memory to code that assumes 32 or 64.
    checks.AC_DEFINE("HAVE_ALIGNED_MALLOC", condition = "ac_cv_aligned_malloc", if_true = "1", if_false = "0"),
    checks.AC_DEFINE("HAVE_SIMD_ALIGN_32", condition = "ac_cv_avx_external", if_true = "1", if_false = "0"),
    checks.AC_DEFINE("HAVE_SIMD_ALIGN_64", condition = "ac_cv_avx512_external", if_true = "1", if_false = "0"),
    ...
    checks.AC_TRY_LINK(define = "HAVE_INT128", code = "volatile __int128 value = 1; value /= 100;"),
    ...
    checks.AC_TRY_COMPILE(define = "HAVE_AS_ARCHEXT_CRC_DIRECTIVE", code = "void foo(void){ __asm__ volatile(\".arch_extension crc\"); }"),
    checks.AC_TRY_COMPILE(define = "HAVE_AS_ARCHEXT_AES_DIRECTIVE", code = "void foo(void){ __asm__ volatile(\".arch_extension aes\"); }"),
    checks.AC_TRY_COMPILE(define = "HAVE_AS_ARCHEXT_SHA3_DIRECTIVE", code = "void foo(void){ __asm__ volatile(\".arch_extension sha3\"); }"),
    ...
    # Windows has no pthreads; FFmpeg's compat/w32pthreads.h wraps the Win32 API.
    checks.AC_DEFINE("HAVE_W32THREADS", condition = "ac_cv_os_windows", if_true = "1", if_false = "0"),
    ...
    checks.AC_TRY_COMPILE(define = "HAVE_X86ASM", code = "#if !(" + _IS_X86_64 + ")\n#error\n#endif"),

    # ========== Public features (HAVE_LIST_PUB) ==========
    # MSVC defines neither macro, so require them to exist before comparing —
    # otherwise "0 != 0" would silently report a big-endian target.
    checks.AC_TRY_COMPILE(define = "HAVE_BIGENDIAN", code = "#if !defined(__BYTE_ORDER__) || !defined(__ORDER_BIG_ENDIAN__) || __BYTE_ORDER__ != __ORDER_BIG_ENDIAN__\n#error\n#endif"),
    ...
    checks.AC_TRY_COMPILE(define = "HAVE_IOCTL_POSIX", code = "int ioctl(int, int, ...);", includes = ["#include <sys/ioctl.h>"]),
