...
FFMPEG_CONFIG_CHECKS = [
    ...
    # 32 bits and a function pointer is 64, so it truncates.
    #
    # Getting this one wrong is not a missing feature but a silent miscompile:
    # av_malloc() falls through to plain malloc() (libavutil/mem.c), which
    ...
    #
    # It was exactly that under the BCR machine's VS 2022: rules_cc_autoconf ran
    # cl.exe without /Fo, so concurrent configurations raced on one intermediate
    # object in the shared execroot. Fixed in rules_cc_autoconf 0.20.0, which is
    # why this is no longer OR'd with windows.h.
