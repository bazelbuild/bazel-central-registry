"""Rule for assembling an FFmpeg install tree from CcInfo providers.

Collects headers and static/shared libraries out of the transitive CcInfo of
the FFmpeg libraries and lays them out under include/ and lib/ so that
downstream consumers can treat the result like a `make install` prefix.
"""
