...
def component_select_srcs(exclusive_map, shared_list):
    ...
    for comp, srcs in exclusive_map.items():
        result = result + select({
            "//:enable_{}_is_true".format(comp): srcs,
            "//conditions:default": [],
        })
...
def _with_defaults_impl(_settings, attr):
    key = "{}-{}".format(attr.os, attr.cpu)
    state = RESOLVED_COMPONENTS.get(key, RESOLVED_DEFAULT)

    return {
        str(Label("//:enable_{}".format(comp))): state.get(comp, False)
