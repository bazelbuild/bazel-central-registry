...
def _gmp_mpn_m4_impl(ctx):
    ...
    if not ctx.attr.disable_asm and _should_skip(operation, ctx.files.asm_srcs, search_dirs, workspace_root):
        out = ctx.actions.declare_file("mpn/" + operation + ".c")
        ctx.actions.write(out, "")
        return DefaultInfo(files = depset([out]))

    asm_src = None if ctx.attr.disable_asm else _find_asm_source(operation, ctx.files.asm_srcs, search_dirs, workspace_root)
    if asm_src:
