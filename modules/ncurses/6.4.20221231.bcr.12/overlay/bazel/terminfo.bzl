...
def _terminfo_directory_impl(ctx):
    ...
    ctx.actions.run(
        ...
        use_default_shell_env = True,  # mkdir
    )
...
_DEFAULT_TERMINFO_PATHS = [ "/usr/share/terminfo", "/run/current-system/sw/share/terminfo" ]
...
def _local_terminfo_repository_impl(rctx):
    ...
    fail_message = None
    for path in rctx.attr.paths:
        terminfo_path = rctx.path(path)
...
local_terminfo_repository = repository_rule(
    ...
    attrs = {
        ...
        "paths": attr.string_list(
            doc = "Search path to local terminfo data.",
            default = _DEFAULT_TERMINFO_PATHS,
        ),
...
_data_tag = tag_class(
    attrs = {
        ...
        "paths": attr.string_list(
            doc = "Search paths to local terminfo data.",
            default = _DEFAULT_TERMINFO_PATHS,
        ),
    },
)
